...
impl<R: Registry> CommandExecutor<R> {
    ...
    pub fn dispatch(&self, command: &str, server: &mut QuartzServer<R>, sender: CommandSender) {
        ...
        let root = match self.commands.get(root_name) {
            Some(root) => &root.0,
            None => {
                context.sender.send_message(Component::colored(
                    format!("No command found named \"{}\"", root_name),
                    PredefinedColor::Red,
                ));
                return;
            }
            ...
            // the sender that some arguments are getting ignored and execute the command
                ...
                context.sender.send_message(Component::colored(
                    format!(
                        ...
                        state.raw_args.remaining_truncated(35)
                    ),
                    PredefinedColor::Red,
                ));
    ...
    fn expect_args(children: &[CommandNode<R>], context: &CommandContext<'_, R>) {
        ...
        context
            .sender
            .send_message(Component::colored(message, PredefinedColor::Red));
    }
    ...
    pub fn command_description(&self, command: &str) -> Option<&String> {
        self.commands
            .get(command)
            .map(|(_cmd, description)| description)
    }
}
...
impl<'cmd> ExecutableCommand<'cmd> {
    ...
    pub fn execute_with<R: Registry>(&self, context: CommandContext<'cmd, R>) {
        context
            .executor
            .dispatch(self.0, context.server, context.sender);
    }
}
...
impl<R: Registry> CommandNode<R> {
        ...
        match self {
            CommandNode::Argument { base, argument, .. } => {
                argument.apply(context, base.name, arg).map_err(|e| {
                    Component::colored(
                        format!("Invalid value for argument \"{}\": {}", base.name, e),
                        PredefinedColor::Red,
                    )
                })?;
            }
            CommandNode::Redirection { base, selector } => match selector(state) {
                ...
                ParserRedirection::Root => {
                    context.arguments.insert(
                        base.name,
                        Argument::Command(ExecutableCommand(state.raw_args.remaining())),
                    );
                }
                ParserRedirection::None => {
                    return Err(Component::colored(
                        "Internal command parser error: failed to redirect".to_owned(),
                        PredefinedColor::Red,
                    ));
                }
            },
    ...
    pub fn add_suggestions<'cmd>(
        &self,
        state: &ParserState<'cmd, R>,
            ...
            CommandNode::Redirection { selector, .. } => match selector(state) {
                ParserRedirection::Node(node) => {
                    for child in node
                        ...
                        .into_iter()
                        .flatten()
                        .filter(|child| child.partial_match(arg))
                    {
